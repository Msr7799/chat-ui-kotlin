...
object ChatStreamingClient {
    ...
    fun chatCompletionStreamWithFiles(
        ...
        // For Google Vertex AI, use native Vertex AI client
        val appContext = context ?: ConfigManager.getAppContext()
        if (providerConfig.provider == ApiProvider.GOOGLE_VERTEX_AI && appContext != null) {
            Log.i(TAG, "Switching to Vertex AI native client for Google")
            
            // Prepare messages asynchronously on IO dispatcher (non-blocking)
            ...
            // Use VertexAIClient
            VertexAIClient.streamGenerateContent(
                context = appContext,
