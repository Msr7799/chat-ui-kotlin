...
class ChatApiClient {
    ...
    suspend fun testConnection(config: com.example.chat_ui.data.ProviderConfig): ApiResult<Unit> = withContext(Dispatchers.IO) {
        try {
            if (config.apiKey.isBlank() && !(config.provider == com.example.chat_ui.data.ApiProvider.GOOGLE_VERTEX_AI && config.useGoogleAuth)) {
            ...
            return@withContext when (config.provider) {
                ...
                com.example.chat_ui.data.ApiProvider.GOOGLE_VERTEX_AI -> {
                    if (config.useGoogleAuth) {
                        ApiResult.Error("Cannot test with Firebase Auth mode enabled")
                    } else {
